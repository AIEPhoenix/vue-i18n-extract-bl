...
describe("file: create-report/index", () => {
  ...
  it("Log report to console", async () => {
    ...
    expect(consoleTableSpy.mock.calls[0][0]).toEqual(
      expectedI18NReport.missingKeys
    );
    expect(consoleTableSpy.mock.calls[1][0]).toEqual(
  ...
  it("Write report to file at output path", async () => {
    options.output = "./test";
    const writeReportSpy: jest.SpyInstance<unknown> = jest.spyOn(
      report,
      "writeReportToFile"
    );
  ...
  it("Should exclude keys if the exclude option was used", async () => {
    const exclude = [
      ...
      "unused_json",
      "unused_nested.auth",
      "unused_nested.forgot",
    ];
    const report = await createI18NReport({ ...options, exclude });
    const expectedI18NReportWithoutExcludedKeys = {
      ...expectedI18NReport,
      unusedKeys: [],
    };
    expect(report).toEqual(expectedI18NReportWithoutExcludedKeys);
  });
  ...
  it("Write missing keys to language files", async () => {
    options.add = true;
    const writeMissingSpy: jest.SpyInstance<unknown> = jest.spyOn(
      languageFileActions,
  ...
  it("Remove unused keys from language files", async () => {
    options.remove = true;
    const removeUnusedSpy: jest.SpyInstance<unknown> = jest.spyOn(
      languageFileActions,
  ...
  it("Only detect missing", async () => {
    const report = await createI18NReport({
      ...options,
      detect: [DetectionType.Missing],
    });
    const expectedI18NReportOnlyDetectingMissing = {
      ...expectedI18NReport,
      unusedKeys: [],
      maybeDynamicKeys: [],
    };

    expect(report).toEqual(expectedI18NReportOnlyDetectingMissing);
  });
});
